...
class FunctionCreateHandler {
	...
	process(options) {
		return BbPromise.try(() => {
			...
			return fs.mkdirsAsync(functionFolder)
				.then(BbPromise.join(
					fs.copyAsync(path.join(templatesFunctionsJsFolder, eventTrigger + '_args.json'), path.join(functionFolder, 'args.json')),
	...
	help() {
		...
		help += '      The event type which triggers this function: http | blob | eventhub | timer | queue | serviceBus' + os.EOL;
	...
	_createFunctionJson(path, eventTrigger, authLevel, connectionStringKey) {
		return BbPromise.try(() => {
			...
			switch (eventTrigger) {
				...
				case 'serviceBus':
					o.disabled = false;
					b.name = 'myServiceBusMessage';
            		b.type = Events.TRIGGER.SERVICEBUS_TRIGGER;
            		b.topicName = 'myTopicName';
            		b.subscriptionName = 'mySubscriptionName';
            		b.connection = connectionStringKey;
					break;
				default:
